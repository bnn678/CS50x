...
{
    char* mime_type;
    mime_type = strrchr(path, '.');
    
    if( strncmp(mime_type, ".css", 4) == 0 )
    {
        return "text/css";
    }
    else if( strncmp(mime_type, ".html", 5) == 0 )
    {
        return "text/html";
    }
    else if( strncmp(mime_type, ".gif", 4) == 0 )
    {
        return "image/gif";
    }
    else if( strncmp(mime_type, ".ico", 4) == 0 )
    {
        return "image/x-icon";
    }
    else if( strncmp(mime_type, ".jpg", 4) == 0 )
    {
        return "image/jpeg";
    }
    else if( strncmp(mime_type, ".js", 3) == 0 )
    {
        return "text/javascript";
    }
    else if( strncmp(mime_type, ".php", 4) == 0 )
    {
        return "text/x-php";
    }
    else if( strncmp(mime_type, ".png", 4) == 0 )
    {
        return "image/png";
    }
    return NULL;
...
{
    // Define method and ensure that it is "GET"
    int i;
    i = strcspn(line," ");
    if( i != 3 )
    {
    ...
    method[3] = '\0';
    //printf(" %s\n %lu\n %i\n", method, strlen(method), strcmp(method, "GET") );
    ...
    // Ensure that request-taret starts with a '/'
    // Ensure that query does not contain any '"'
    char* pch;
    int beginning; int end;
    pch=strchr(line,' ');
    beginning = pch-line+2;
    pch=strchr(pch+1,' ');
    end = pch-line+1;
    char* request = malloc( sizeof(char) * (end-beginning+1) );
    for( int i = 0; i < end-beginning; i++ )
    {
        request[i] = line[i+beginning-1];
    }
    ...
    char l;
    for( int i = 0, lim = strlen(request); i < lim; i++ )
    {
        l = request[i];
        if( l == '"' )
        {
    ...
    // Define HTTP version and ensure HTTP version is 1.1\r\n
    pch=strrchr(line,'/');
    char* version = malloc( sizeof(char) * 9);
    for( int i = 0; i < 8; i++ )
    {
        version[i] = pch[i+1];
    }
