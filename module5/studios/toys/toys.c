...
int main(void)
{
    ...
    while(true)
    {
        ...
        printf("What would you like to name the new toy: \n");
        ...
        int hashkey = hashByFirstLetter(toynode->name);
        ...
        else
        {
            ...
            current = table[hashkey];
            while( current->next != NULL )
            {
                current = current->next;
            }
            current->next = toynode;
        }

        // now we report what happened
        printf("OK, %s has been added to bin %i:", toynode->name, hashkey);
